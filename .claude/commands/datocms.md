...
The API token should be in the environment variable `DATOCMS_API_TOKEN`. If it is not set, ask the user to provide it or set it with:
```
...
Based on the user's request, perform one or more of the following operations using `curl` via the Bash tool.
...
1. If the user hasn't specified a model ID, first **list models** to find the right one.
2. Confirm the field structure needed before creating records.
3. After creating, show the user the new record ID and its URL in the DatoCMS dashboard: `https://YOUR_PROJECT.admin.datocms.com/editor/item_types/MODEL_ID/items/ITEM_ID/edit`
4. Ask the user whether to publish immediately or leave as draft.

## Error handling

- `401 Unauthorized`: Token is missing or invalid — check `$DATOCMS_API_TOKEN`
- `422 Unprocessable Entity`: Field validation failed — check the field names and types match the model schema
- `404 Not Found`: Wrong item ID or model ID
