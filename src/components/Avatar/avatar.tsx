...
export const PureAvatar: FunctionComponent<PureAvatarProps> = ({
  ...
  return (
    <div className={avatarSizeHeight}>
      <div className={`px-4 flex items-center flex-shrink-0 text-white ${className}`}>
        <Link to="/" className="flex flex-row items-center text-2xl text-gray-400">
          <GatsbyImage
          ...
          />
          {title && (
            <span className="ml-4 text-xl font-semibold tracking-tight" data-testid="avatar-title">
              {title}
            </span>
          )}
        </Link>
